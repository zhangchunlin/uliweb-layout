                ...
                $(el, this).each(function() {
                    if (this.id || $(this).is("input:radio")) {
                        if ($(this).hasClass("ue")) {
                            ...
                            formJson[this.name + "_DESC"] = $(this).siblings('label').text().replace("：","");
                        } else if ($(this).is("input:checkbox")) {
                            if ($(this).is(":checked")) formJson[this.id] = $(this).val();
                            formJson[this.id + "_DESC"] = $(this).siblings('label').text().replace("：","");
                ...
                if ("string" === typeof(item)) {
                    ...
                    if ($(this).find(":radio[name=" + item + "],:checkbox[name=" + item + "],:radio[id=" + item + "],:checkbox[id=" + item + "]").length > 0) {
                        $(this).find(":radio[name=" + item + "][value=" + value + "]," +
                            ...
                            ":checkbox[id=" + item + "][value=" + value + "]").prop("checked", true);
                    }
    ...
    $.fn.selectOnChange = function(option, control, extOption) {
        ...
        this.setSelect2 = function(node, control, flag) {
            ...
            if (extSettings.keyMap) {
                ...
                if ($.isString(extSettings.keyMap)) {
        ...
        this.setRadio = function(node, control, flag) {
            ...
            $("option[value!=]", node).each(function(index) {
                ...
                $(":radio", tnode).attr("id", node.attr("id") + "_" + index).attr("name", node.attr("id"))
                    .val($(this).val()||$(this).text());
                if ($(this).val() == value) {
                    $(":radio", tnode).attr("checked", true);
                }
                $("label", tnode).html($(this).text()).attr("for", node.attr("id") + "_" + index);
                tList.push(tnode);
            })
            ...
            if (tList.length > 0) {
                ...
                if (node.is(":disabled")) {
                	$.each(tList,function(){
                		$(":radio", this).attr("disabled",true);
                	})
                }
                if (node.is("[readonly]")) {
                	$.each(tList,function(){
                		$(":radio", this).attr("readonly",true);
                	})
                }
                ...
                if (!value) {
                    if (extSettings.singleSelect && tList.length == 1) {
                        $(":radio", tList[0]).attr("checked", true);
    ...
    $.fn.setDisabled = function(option) {
        ...
        $(this).each(function(index, el) {
            ...
            if (setting.ue) {
                ...
                node.each(function() {
                    UE.getEditor(this.id).setDisabled("fullscreen");
                });
            }
